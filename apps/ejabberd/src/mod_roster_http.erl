...
-export([start/2, stop/1,
	 process_iq/3,
	 process_local_iq/3,
	 get_user_roster/2, %% TODO - dis
	 get_subscription_lists/3, %% TODO - dis
	 in_subscription/6,
	 out_subscription/4,
	 set_items/3,
	 remove_user/2,
...
roster_versioning_enabled(Host) ->
    gen_mod:get_module_opt(to_binary(Host), ?MODULE, versioning, false).

roster_version_on_db(Host) ->
    gen_mod:get_module_opt(to_binary(Host), ?MODULE, store_current_id, false).
...
%% [{roster,{<<"joe">>,<<"localhost">>,{<<"joe2">>,<<"localhost">>,<<>>}},{<<"joe">>,<<"localhost">>},{<<"joe2">>,<<"localhost">>},<<>>,both,none,[<<"wiredcraft">>],<<>>,[]},{roster,{<<"joe">>,<<"localhost">>,{<<"joe3">>,<<"localhost">>,<<>>}},{<<"joe">>,<<"localhost">>},{<<"joe3">>,<<"localhost">>},<<>>,both,none,[<<"wiredcraft">>],<<>>,[]},{roster,{<<"joe">>,<<"localhost">>,{<<"joe4">>,<<"localhost">>,<<>>}},{<<"joe">>,<<"localhost">>},{<<"joe4">>,<<"localhost">>},<<>>,both,none,[<<"wiredcraft">>],<<>>,[]}]
%% [{roster,{<<"joe">>,<<"localhost">>,{<<"joe2">>,<<"localhost">>,<<>>}},{<<"joe">>,<<"localhost">>},{<<"joe2">>,<<"localhost">>,<<>>},<<>>,both,none,[<<"testgroup">>],<<>>,[]}]
get_user_roster(Acc, US) ->
	%% US is {<<"joe">>,<<"localhost">>}
	{User, Host} = US,
	UserRoster = get_github_roster_list(to_binary(User), to_binary(Host)) ++ Acc,
	?DEBUG("User Roster: ~p", [UserRoster]),
	UserRoster.

get_github_roster_list(User, Host) ->
	case get_github_list(to_list(User), to_list(Host)) of
		{ok, Items} when is_list(Items) ->
			mjson_list_obj_to_roster_item([], User, Host, Items);
		_ ->
		    []
    end.

%% NOTE: need to merge? maybe
mjson_list_obj_to_roster_item(Acc, U, H, []) ->
	Acc;
mjson_list_obj_to_roster_item(Acc, U, H, [{struct, R}|Rest]) ->
	?DEBUG("mjson_list_obj_to_roster_item: ~p", [R]),
	case mjson_list_validate(R) of
		true ->
			mjson_list_obj_to_roster_item(Acc ++ [#roster {
				usj = {U, H, {
					proplists:get_value(<<"username">>, R),
					proplists:get_value(<<"host">>, R),
					<<>>
				}},
				us = {U, H},
				jid = {
				...
				name = <<>>,subscription = both,ask = none,
				groups = proplists:get_value(<<"groups">>, R),
				askmessage = <<>>,
				xs = []
			}], U, H, Rest);
		false ->
			mjson_list_obj_to_roster_item(Acc, U, H, Rest)
	end.

%% simple validation
mjson_list_validate(R) ->
	?DEBUG("mjson_list_validate: ~p", [lists:subtract(proplists:get_keys(R), 
		[<<"username">>, <<"host">>, <<"groups">>])]),
	case lists:subtract(proplists:get_keys(R), 
		[<<"username">>, <<"host">>, <<"groups">>]) of
		[] ->
			true;
		_ ->
			false
...
%    US = {LUser, LServer},
    ...
    SubList = case get_github_roster_list(to_binary(User), to_binary(Server)) of
		Items when is_list(Items) ->
		    fill_subscription_lists(JID, Items, [], [], []);
		_ ->
		    {[], [], []}
    end,
    ?DEBUG("Got subscription list: ~p", [SubList]),
    SubList.

%% Currently Host == Org? not good. We can map in the config
get_github_list(User, Host) ->
	Org = case gen_mod:get_module_opt(to_binary(Host), ?MODULE, roster_host_to_org, undefined) of
		undefined ->
			Host;
		O ->
			proplists:get_value(Host, O)
	end,
	Url = bind_params(gen_mod:get_module_opt(to_binary(Host), ?MODULE, roster_http_url, undefined), Org, User),
	%% FIXME, We need TLS/SSL
	Timeout = gen_mod:get_module_opt(to_binary(Host), ?MODULE, roster_http_timeout, ?DEFAULT_HTTP_TIMEOUT),
    case httpc:request(get, {Url, [{"Accept", "application/json"}]}, [{timeout, Timeout}], [{body_format, binary}]) of
		{ok, {_Response, Headers, Res}} -> %% FIXME: check http result
			case catch mochijson2:decode(Res) of
				{'EXIT', _} ->
					?ERROR_MSG("Error decoding JSON string: ~p", [Res]),
					{error, json_decode_error};
				Struct ->
					?DEBUG("JSON structs: ~p", [Struct]),
					{ok, Struct}
			end;
		{error, Reason} ->
			?ERROR_MSG("Get request failed: ~p~n  Url: ~p", [Reason, Url]),
			{error, http_get}
	end.

bind_params(Url, _, _) when Url == undefined ->
	?ERROR_MSG("roster_http_url not found!", []),
	[];
bind_params(Url, Org, User) when is_binary(Url) ->
	bind_params(to_list(Url), Org, User);
bind_params(Url, Org, User) when is_list(Url) ->
	re:replace(re:replace(Url, ":org", Org, [{return, list}]), ":user", User, [{return, list}]).
